...
var log = clog.FromContext(context.Background())
...
func (nt *nodeTree) addNode(n *v1.Node) {
	...
	if na, ok := nt.tree[zone]; ok {
		for _, nodeName := range na {
			if nodeName == n.Name {
				log.V(0).Info("Node already exists in the NodeTree", "node", tools.KObj(n))
				return
			}
		}
	...
	log.V(2).Info("Added node in listed group to NodeTree", "node", tools.KObj(n), "zone", zone)
	nt.numNodes++
}
	...
	if na, ok := nt.tree[zone]; ok {
		for i, nodeName := range na {
			if nodeName == n.Name {
				...
				log.V(2).Info("Removed node in listed group from NodeTree", "node", tools.KObj(n), "zone", zone)
				nt.numNodes--
				return nil
			}
		}
	}
	log.Error(nil, "Node in listed group was not found", "node", tools.KObj(n), "zone", zone)
